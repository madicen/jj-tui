...
func (m *Model) renderPRs() string {
	if !m.githubService {
		noGitHub := []string{
			...
			"",
			"1. Sign in via Settings (,) → GitHub (device flow), set GITHUB_TOKEN, or run: gh auth login",
			"   Order: saved jj-tui token, then GITHUB_TOKEN, then `gh auth token` (GitHub CLI).",
			"",
