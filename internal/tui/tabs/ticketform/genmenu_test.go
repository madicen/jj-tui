...
// TestGenMenuLifecycle mirrors descedit's lifecycle test for the ticket form.
func TestGenMenuLifecycle(t *testing.T) {
	...
	v, _, _ := m.MenuOverlay()
	if !strings.Contains(v, "smart") {
		t.Fatalf("overlay should include the smart profile:\n%s", v)
	}
