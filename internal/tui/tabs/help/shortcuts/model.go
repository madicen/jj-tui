...
func (m Model) lines() []string {
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("z"), styles.HelpDescStyle.Render("split (experimental, when shown): jj evolog parent + step file list; o step diff; s AI-suggest row; if AI says no split, Enter twice at same row (or move j/k) before split; d optional AI describe @-/@ after split (needs AI in Settings); moves change (and feature bookmark if present)")))
