...
func (m Model) lines() []string {
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("z"), styles.HelpDescStyle.Render("split (experimental, when shown): jj evolog parent + step file list; o opens colored step diff overlay; moves change (and feature bookmark if present)")))
