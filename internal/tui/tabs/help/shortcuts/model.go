...
func (m Model) lines() []string {
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("^g"), styles.HelpDescStyle.Render("Generate description with AI (optional; Settings → Advanced: provider + API key or JJ_TUI_AI_API_KEY)")))
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("z"), styles.HelpDescStyle.Render("split (experimental, when shown): jj evolog parent + step file list; o patch; s/^g AI row (✨ top-right); if AI says no split, Enter twice at same row (or move j/k) before split; d optional AI describe @-/@ after split (needs AI in Settings); moves change (and feature bookmark if present)")))
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("^g"), styles.HelpDescStyle.Render("Suggest bookmark name with AI (new bookmark name field only)")))
	lines = append(lines, "")
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("^g"), styles.HelpDescStyle.Render("Generate title and body with AI")))
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("Esc"), styles.HelpDescStyle.Render("Cancel")))
	...
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("^g"), styles.HelpDescStyle.Render("Generate title and description with AI (graph revision or @)")))
	lines = append(lines, fmt.Sprintf("  %s  %s", styles.HelpKeyStyle.Width(helpKeyColW).Render("Esc"), styles.HelpDescStyle.Render("Cancel")))
